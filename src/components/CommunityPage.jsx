...
function CommunityOverview({ profiles, hiddenCount, onOpenProfile, onOpenMenu }) {
...
function CommunityProfile({
  ...
  return (
    <section className="community-page">
      ...
      <div className="safety-actions community-safety-actions">
        <button type="button" onClick={() => onToggleProfileFollow(profile.handle)}>
          {isFollowingProfile ? "Profil nicht mehr folgen" : "Profil folgen"}
        </button>
        <button type="button" onClick={() => onReport({
          kind: "community-profile",
          key: `community:${profile.id}`,
          label: `${profile.name} (Community-Profil)`
        })}>
          Profil melden
        </button>
      ...
      <div className="community-garage">
        ...
        {profile.vehicles.map((vehicle) => (
          <article className="community-vehicle-card" key={vehicle.id}>
            ...
            <button
              className={followingVehicleIds.includes(vehicle.id) ? "vehicle-follow-button active" : "vehicle-follow-button"}
              type="button"
              onClick={() => onToggleVehicleFollow(vehicle.id, vehicle.name)}
            >
              {followingVehicleIds.includes(vehicle.id) ? "Fahrzeug abonniert" : "Fahrzeug folgen"}
            </button>
...
export default function CommunityPage({
  ...
  function handleToggleBlock(key, label) {
    const wasBlocked = blockedKeys.has(key);
    onToggleBlock(key, label);
    if (!wasBlocked) {
      ...
      <div className="screen-page community-screen-page">
        {activeProfile ? (
          <CommunityProfile
            ...
            isBlocked={blockedKeys.has(`community:${activeProfile.id}`)}
            isFollowingProfile={followingHandles.includes(activeProfile.handle)}
            followingVehicleIds={followingVehicleIds}
            onToggleProfileFollow={onToggleProfileFollow}
            onToggleVehicleFollow={onToggleVehicleFollow}
          ...
          <CommunityOverview
            ...
            hiddenCount={communityProfiles.length - visibleProfiles.length}
